...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        for j in range(len(output)):
            if args.ltc:
                if j != len(output)-1:
                    loss += cascade_loss(output[j], output[j+1], target_var, args.w, args.C)
...
def cascade_loss(output_0, output_1, label, w, C):
    conf_0, pred_0 = torch.max(F.softmax(output_0, dim=1), 1)
    incorrect_idx_0 = (pred_0 != label)
    _, pred_1 = torch.max(output_1, 1)
    incorrect_idx_1 = (pred_1 != label)
    conf = conf_0.view(-1)
    loss = conf*incorrect_idx_0.float() + (1-conf)*(incorrect_idx_1.float() + C)
    loss = loss.mean() * w
